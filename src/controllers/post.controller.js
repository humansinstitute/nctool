import {
  ...
  buildTextNote,
} from "../services/nostr.service.js";
...
export const sendNoteController = asyncHandler(async (req, res) => {
  ...
  const events = await ndk.fetchEvents(filter, {
    timeoutSec: DEFAULT_TIMEOUT / 1000,
  });
...
export const sendNoteRemoteController = asyncHandler(async (req, res) => {
  const { senderNpub, callNpub, responseNpub, signerNpub, noteContent } =
    req.body;
  const powBits = DEFAULT_POW;
  ...
  if (
    ...
    return res
      ...
      .json({
        error:
          "senderNpub, callNpub, responseNpub, signerNpub and noteContent are required",
      });
  }
  const callID = uuidv4();
  ...
  try {
    ...
    console.log(
      "DEBUG → unsignedEvent (with pubkey, before PoW):",
      unsignedEvent
    );
  } catch (e) {
    return res
      .status(400)
      .json({ error: "Invalid signerNpub provided", message: e.message });
  }
  ...
  console.log("Message send to encryption: ", message);
